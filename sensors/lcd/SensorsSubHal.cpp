...
SensorsSubHal::SensorsSubHal() : mCallback(nullptr), mNextHandle(1) {
    if (property_get_bool("ro.vendor.sensors.xiaomi.double_tap", false)) {
        AddSensor<LcdDoubleTapSensor>();
    }
    if (property_get_bool("ro.vendor.sensors.xiaomi.single_tap", false)) {
        AddSensor<LcdSingleTapSensor>();
    }
    if (property_get_bool("ro.vendor.sensors.xiaomi.pen_detach", false)) {
        AddSensor<LcdPenDetachSensor>();
    }
}
...
Return<void> SensorsSubHal::debug(const hidl_handle& fd, const hidl_vec<hidl_string>& args) {
    ...
    if (args.size() != 0) {
        fprintf(out,
                "Note: sub-HAL %s currently does not support args. Input arguments are ignored.\n",
                getName().c_str());
    }
